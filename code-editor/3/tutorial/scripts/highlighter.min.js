...
var code=element.textContent;var env={element:element,language:language,grammar:grammar,code:code};function insertHighlightedCode(highlightedCode){env.highlightedCode=highlightedCode;_.hooks.run('before-insert',env);var language=env.language.toLowerCase()
var langProvider=(["c++","cpp"].includes(language)?cppLanguage:language==="css"?cssLanguage:["html","xhtml"].includes(language)?htmlLanguage:language==="java"?javaLanguage:["javascript","ecmascript","js","node"].includes(language)?javascriptLanguage:["json","json5"].includes(language)?jsonLanguage:language==="jsx"?javascriptLanguage.configure({dialect:"jsx"}):language==="markdown"?markdownLanguage:["php","php3","php4","php5","php7","phtml"].includes(language)?phpLanguage:language==="python"?pythonLanguage:language==="rust"?rustLanguage:language==="tsx"?javascriptLanguage.configure({dialect:"jsx ts"},"typescript"):["typescript","ts"].includes(language)?javascriptLanguage.configure({dialect:"ts"},"typescript"):language==="webassembly"?wastLanguage:["xml","rss","wsdl","xsd"].includes(language)?xmlLanguage:null);env.element.innerHTML=renderString(env.highlightedCode,{langProvider}).codeLinesOnly;_.hooks.run('after-highlight',env);_.hooks.run('complete',env);callback&&callback.call(env.element)}
